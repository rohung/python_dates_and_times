...
logging.basicConfig(filename='time_trakcer.log', level=logging.DEBUG)
...
def calculate_totals_menu():
    ...
    if client in utils.return_clients():
        ...
        range_choice = input("Make your choie: ")
        if range_choice == "1":
...
def date_range_menu(client):
    ...
    try:
        ...
        logging.info('client: {}; split_dates: {}'.format(client, split_dates))
    except ValueError:
...
def start_tracking_menu():
    ...
    if utils.check_if_job_running():
        ...
        print("Stopping job...")
        end_time = utils.time_stamp_now()
        functions.stop_tracking(end_time)
        print("Previous job stopped")
        print("=========================")
    print("Who is the client?")
    utils.list_clients()
    start_time = utils.time_stamp_now()
    client = input("Enter one of the above or a new client: ")
    description = input("Enter a short description: ")
    functions.start_tracking(client, description, start_time)
    print("=========================")
    utils.back_to_main_menu()
